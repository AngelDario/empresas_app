...
Widget _buildHistoryButton(
    BuildContext context, String clientId, String type) {
  final db = FirebaseFirestore.instance;
                        ...
                        return AlertDialog(
                          ...
                          actions: <Widget>[
                            ...
                            TextButton(
                              onPressed: () {
                                final db = FirebaseFirestore.instance;
                                db.collection('services').doc(document.id).update({
                                  'status': 'cancelled',
                                });
                                Navigator.of(context).pop();
                              },
      ...
      body: SafeArea(
        child: Column(
          ...
          children: [
            Padding(
              padding:
                  const EdgeInsets.only(left: 16.0, top: 13, right: 16.0, bottom: 13),
              child: Text(
                widget.type == 'pending' ? 'Servicios pendientes' : 
                widget.type == 'accepted' ? 'Servicios aceptados' :
                widget.type == 'cancelled' ? 'Servicios cancelados' :
                widget.type == 'finished' ? 'Servicios finalizados' : '',
